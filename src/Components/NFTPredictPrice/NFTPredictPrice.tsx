...
const NFTPredictPrice = ({ nftListing, nftOffer }: INFTPredictPriceProps) => {
  ...
  useEffect(() => {
    ...
    let listingData = listingPrices.map(
      (price) => {
        return {x:1, y:price}
      }
    );

    let offerData = offerPrices.map(
      (price) => {
    ...
    if (listingPrices.length == 1 && offerPrices.length >= 2){
      if(listingPrices[0] >= Math.min(...offerPrices) && listingPrices[0] <= Math.max(...offerPrices)) {
        predictData.push({x:2, y:listingPrices[0]})
      }
    }
    if (listingPrices.length >= 2 && offerPrices.length == 1){
      if(offerPrices[0] >= Math.min(...listingPrices) && offerPrices[0] <= Math.max(...listingPrices)) {
        predictData.push({x:2, y:offerPrices[0]})
      }
    }
    if (listingData.length >= 2 && offerData.length >= 2) {
      if(Math.min(...listingPrices) >= Math.min(...offerPrices) && Math.min(...listingPrices) <= Math.max(...offerPrices)) {
        predictData.push({x:2, y:Math.min(...listingPrices)})
        predictData.push({x:2, y:Math.max(...offerPrices)})
      }
      if(Math.max(...listingPrices) >= Math.min(...offerPrices) && Math.max(...listingPrices) <= Math.max(...offerPrices)) {
